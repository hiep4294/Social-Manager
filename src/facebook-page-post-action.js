...
function analyzePublishRequest(response) {
  try {
...
export async function executePostPage(page, payload, db) {
  ...
  try {
    ...
    const publishRequestEvidence = analyzePublishRequest(publishResponse);
    ...
    const publishEvidence = extractPublishEvidence(publishPayload, target.url);

    try {
      const diagnosticDir = path.resolve(process.cwd(), 'data');
      fs.mkdirSync(diagnosticDir, { recursive: true });
      fs.writeFileSync(
        path.join(diagnosticDir, 'facebook-last-publish-response.json'),
        JSON.stringify({
          captured_at: new Date().toISOString(),
          page_url: target.url,
          requested_image: Boolean(payload.image_url),
          dedupe_marker: String(payload.dedupe_marker || ''),
          request: publishRequestEvidence,
          response_evidence: publishEvidence,
    ...
    if (
      payload.image_url &&
      !publishRequestEvidence.has_media_reference
    ) {
      throw Object.assign(
        new Error('ComposerStoryCreateMutation không chứa media attachment cho bài yêu cầu ảnh; không xác nhận thành công'),
        { code: 'NEEDS_REVIEW' }
      );
    }
    ...
    let verified = !marker;
    let verifiedUrl = '';
    let mediaVerifiedByDirectUrl = false;

    // Prefer the exact story/post URL returned by GraphQL. Feed scanning is
    // only a fallback because Page feeds can lag or reorder newly created
    // stories.
    for (const candidateUrl of publishEvidence.urls) {
      if (verified) break;

      const direct = await verifyPublishedTarget(
        page,
        candidateUrl,
        marker,
      ...
      if (direct.verified) {
        ...
        mediaVerifiedByDirectUrl = Boolean(direct.media_verified);
        break;
      }
    }
    ...
    if (!verified) {
      const evidenceText = publishEvidence.urls.length
        ? `; GraphQL URLs=${publishEvidence.urls.slice(0, 3).join(' | ')}`
        : publishEvidence.ids.length
          ? `; GraphQL IDs=${publishEvidence.ids.slice(0, 5).map(item => item.value).join(' | ')}`
          : '';

      throw Object.assign(
        new Error(
          payload.image_url
            ? 'ComposerStoryCreateMutation thành công và request có media, nhưng chưa xác minh trực tiếp được bài kèm ảnh; không tự retry để tránh đăng trùng' + evidenceText
            : 'ComposerStoryCreateMutation thành công nhưng chưa xác minh trực tiếp được bài; không tự retry để tránh đăng trùng' + evidenceText
        ),
        { code: 'NEEDS_REVIEW' }
      );
    }
    ...
    return {
      ...
      mutation_has_media_reference: Boolean(publishRequestEvidence.has_media_reference),
      media_verified: Boolean(payload.image_url ? (mediaVerifiedByDirectUrl || verified) : true),
      publish_story_ids: publishEvidence.ids,
      publish_story_urls: publishEvidence.urls,
      post_url: verifiedUrl || publishEvidence.urls[0] || null,
      page_name: target.name || payload.page_name || '',
      page_url: target.url,
      current_url: page.url()
    };
...
, depth = 0) => {
      ...
      for (const [key, child] of Object.entries(value)) {
        ...
        if (/photo|image|media|attachment|upload|asset/i.test(key)) {
          ...
          hints.push({ path: nextPath, meaningful, summary });
        }
    ...
    return {
      friendly_name: String(params.get('fb_api_req_friendly_name') || ''),
      doc_id: String(params.get('doc_id') || ''),
      variables_length: variablesRaw.length,
      has_media_reference: hints.some(item => item.meaningful),
      media_hints: hints
    };
...
function extractPublishEvidence(payload, targetUrl = '') {
  const urls = [];
  const ids = [];
  ...
  const addUrl = value => {
    ...
    if (!/facebook\.com/i.test(text)) return;
    if (!urls.includes(text)) urls.push(text);
  };

  const addId = (key, value, currentPath) => {
    const text = String(value ?? '').trim();
    if (!text) return;

    if (
      /legacy_story_hideable_id|post_id|story_id|feedback_id|^id$/i.test(key) &&
      /story|post|create|feedback/i.test(currentPath)
    ) {
      if (!ids.some(item => item.value === text && item.path === currentPath)) {
        ids.push({ path: currentPath, key, value: text });
      }
    }
  };
...
, depth = 0) => {
    ...
    for (const [key, child] of Object.entries(value)) {
      ...
      if (typeof child === 'string' || typeof child === 'number') {
        if (/url|permalink|uri/i.test(key)) addUrl(child);
        addId(key, child, nextPath);
      }
  ...
  const pageIdMatch = String(targetUrl || '').match(/[?&]id=(\d+)/);
  ...
  if (pageId) {
    for (const item of ids) {
      ...
      if (/^\d+$/.test(text)) {
        addUrl(`https://www.facebook.com/${pageId}/posts/${text}`);
      }

      const pair = text.match(/^(\d+)_(\d+)$/);
      if (pair) {
        addUrl(`https://www.facebook.com/${pair[1]}/posts/${pair[2]}`);
      }
    }
  }
...
async function verifyPublishedTarget(page, url, marker, wantsImage) {
  try {
    ...
    const articles = page.locator('[role="article"]');
    const count = Math.min(30, await articles.count().catch(() => 0));

    for (let i = 0; i < count; i += 1) {
      const article = articles.nth(i);
      const text = String(await article.innerText().catch(() => '')).replace(/\s+/g, ' ');

      if (marker && !text.includes(marker)) continue;

      if (!wantsImage) {
        return { verified: true, media_verified: true };
      }
      ...
      if ((await photoLinks.count().catch(() => 0)) > 0) {
        return { verified: true, media_verified: true };
      }
    }

    const body = String(await page.locator('body').innerText().catch(() => '')).replace(/\s+/g, ' ');
    if (!wantsImage && marker && body.includes(marker)) {
      return { verified: true, media_verified: true };
    }
  } catch {}

  return { verified: false, media_verified: false };
}
