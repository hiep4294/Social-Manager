...
export async function executePostPage(page, payload, db) {
  ...
  try {
    ...
    const publishRequestSummary = summarizeGraphqlMutationRequest(publishResponse);
    ...
    const publishResponseSummary = summarizePublishPayload(publishPayload);
    const publishDiagnosticPath = writePublishDiagnostic({
      dedupe_marker: String(payload.dedupe_marker || ''),
      page_url: target.url,
      requested_image: Boolean(payload.image_url),
      request: publishRequestSummary,
      response: publishResponseSummary,
      http_ok: publishResponse.ok(),
      graphql_errors: publishErrors.slice(0, 5).map(error => ({
    ...
    if (!verified) {
      const diagnosticSuffix = publishDiagnosticPath
        ? `; diagnostic=${publishDiagnosticPath}; mutation_media=${publishRequestSummary.has_media_reference ? 'yes' : 'no'}`
        : `; mutation_media=${publishRequestSummary.has_media_reference ? 'yes' : 'no'}`;

      throw Object.assign(
        new Error(
          payload.image_url
            ? (
                publishRequestSummary.has_media_reference
                  ? 'ComposerStoryCreateMutation thành công, request có media nhưng chưa xác minh thấy bài kèm ảnh trên timeline; không tự retry để tránh đăng trùng' + diagnosticSuffix
                  : 'ComposerStoryCreateMutation thành công nhưng request không chứa media attachment; bài có thể chỉ có chữ; không tự retry để tránh đăng trùng' + diagnosticSuffix
              )
            : 'ComposerStoryCreateMutation thành công nhưng chưa xác minh thấy bài trên timeline; không tự retry để tránh đăng trùng' + diagnosticSuffix
        ),
        { code: 'NEEDS_REVIEW' }
      );
    }
    ...
    return {
      ...
      mutation_has_media_reference: Boolean(publishRequestSummary.has_media_reference),
      publish_diagnostic: publishDiagnosticPath || null,
      publish_response_url: publishResponseSummary.direct_url || null,
      post_url: verifiedUrl || publishResponseSummary.direct_url || null,
      page_name: target.name || payload.page_name || '',
      page_url: target.url,
      current_url: page.url()
    };
...
, depth = 0, underMedia = false) {
    if (depth > 10 || mediaHints.length >= 80) return;

    if (Array.isArray(value)) {
      if (underMedia && value.length) {
        mediaHints.push({ path: pathName, type: 'array', length: value.length });
      }
      value.slice(0, 20).forEach((item, index) => {
        walk(item, `${pathName}[${index}]`, depth + 1, underMedia);
      });
      return;
    }
    ...
    if (value && typeof value === 'object') {
      for (const [key, child] of Object.entries(value)) {
        ...
        const mediaKey = /attachment|photo|image|media|upload|asset/i.test(key);
        const nextUnderMedia = underMedia || mediaKey;

        if (mediaKey) {
          if (Array.isArray(child)) {
            mediaHints.push({ path: nextPath, type: 'array', length: child.length });
          } else if (child && typeof child === 'object') {
            mediaHints.push({ path: nextPath, type: 'object', keys: Object.keys(child).slice(0, 20) });
          } else if (child !== null && child !== undefined && child !== '' && child !== false) {
            mediaHints.push({ path: nextPath, type: typeof child, value: String(child).slice(0, 180) });
          }
        }

        walk(child, nextPath, depth + 1, nextUnderMedia);
        if (mediaHints.length >= 80) break;
      }
      return;
    }

    if (underMedia && value !== null && value !== undefined && value !== '' && value !== false) {
      mediaHints.push({ path: pathName, type: typeof value, value: String(value).slice(0, 180) });
    }
  }

  walk(variables);

  const meaningfulMediaHint = mediaHints.some(hint => {
    if (hint.type === 'array') return Number(hint.length || 0) > 0;
    if (hint.type === 'object') return Array.isArray(hint.keys) && hint.keys.length > 0;
    return String(hint.value || '').trim().length > 0;
  });
  ...
  return {
    ...
    has_media_reference: meaningfulMediaHint,
    media_hints: mediaHints.slice(0, 80)
  };
}
...
, depth = 0) {
    if (depth > 12 || hints.length >= 120) return;
    ...
    if (value && typeof value === 'object') {
      for (const [key, child] of Object.entries(value)) {
        ...
        if (
          /story|post|permalink|legacy_story_hideable_id|url|^id$/i.test(key) &&
          (typeof child === 'string' || typeof child === 'number')
        ) {
          const text = String(child).slice(0, 500);
          hints.push({ path: nextPath, value: text });

          if (
            !directUrl &&
            /(?:https?:\/\/[^\s"]*facebook\.com|\/posts\/|story_fbid=|permalink\.php)/i.test(text)
          ) {
            directUrl = text;
          }
        }
...
function writePublishDiagnostic(data) {
  try {
    const dataDir = path.resolve(process.cwd(), 'data');
    fs.mkdirSync(dataDir, { recursive: true });
    const file = path.join(dataDir, 'facebook-last-publish-diagnostic.json');
