  ...
  function baselineExisting() {
    ...
    messages.forEach((message, index) => {
      ...
      for (const run of item.runnable) {
        baseline.add(item.key + ':' + hashString(run.parsed.shell + '\n' + run.parsed.command));
      }
    });
  }
  ...
  async function scan() {
    ...
    try {
      ...
      for (let index = 0; index < messages.length; index++) {
        ...
        const executionKey = item.key + ':' + commandHash;
        const sessionKey = 'kods_exec_' + hashString(executionKey);
        activeCandidateKeys.add(item.key);

        if (baseline.has(executionKey)) continue;
        if (executed.has(executionKey) || sessionStorage.getItem(sessionKey) === '1') continue;
        ...
        candidates.delete(item.key);
        const resultId = hashString(executionKey + ':' + Date.now());
