...
assert.equal(text.value.imageMode, null);
assert.equal(text.value.imageTone, 'auto');
assert.equal(text.value.scheduledAt, null);

const autoImage = normalizeBridgeCommand({
  id: 'cmd-auto-image',
  action: 'post_facebook',
  brand_id: 1,
...
assert.equal(autoImage.ok, true);
assert.equal(autoImage.value.imageMode, 'auto');
assert.equal(autoImage.value.imageTone, 'greeting');
...
assert.equal(imageScheduled.value.imageMode, null);
